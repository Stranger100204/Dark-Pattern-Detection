...
const CONFIRMSHAME_KEYWORDS = [
    ...
    "no, i don't want",
    "i like missing out",
    "no thanks, i'm good with paying full"
];
...
function detectForcedContinuity(elements, url) {
    ...
    elements.forEach(el => {
        ...
        if (keywordMatch(text, CONFIRMSHAME_KEYWORDS)) {
            confirmShamePresent = true;
        }
    ...
    if (subscriptionContext) {
        ...
        if (confirmShamePresent) {
            signals.confirmshaming = true;
            reasons.push("Confirmshaming language detected on decline option");
        }
